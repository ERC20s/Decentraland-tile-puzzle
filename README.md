...
- Reset button. The Reset button in src/ui.tsx calls ResetBoard, which undoes the round: it puts back the exact scramble the current round started from (captured in roundStart after setup and after every Shuffle), clears the selection and zeroes the move count. It never deals a new board — that is what Shuffle is for — and never restores originalImages, so it cannot hand the player a solved picture or fire the win reward.
...
- Win detection and reward. When every tile's image matches its original position (checkIfOriginalImages), the puzzle calls Reward() in src/reward.ts, which spawns a single reusable reward entity, plays a win song (music/champ2.mp3), and shows a grass model. The song restarts on every win, not only the first: the reused entity's AudioSource is set to playing = false and back to playing = true on the next engine frame (a one-shot system that removes itself), because writing true over an already-true field is a no-op the runtime never hears. The reward is a small 4x4 metre grass patch near the north-east corner (REWARD_POSITION { x: 24, y: 0.02, z: 24 } and REWARD_SCALE 0.25 in src/reward.ts), not the scene-wide plate it used to be; src/scene-bounds.test.ts derives the playable footprint from the parcels in scene.json and checks the grass, the machine and the reward all fall inside it.
